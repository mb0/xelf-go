...
func TestRead(t *testing.T) {
	tests := []struct {
		...
		{&List{Vals: []Val{Num(1), Num(2), Num(3)}}, `[1,2,3]`, `[1 2 3]`, ``},
		{&List{Vals: []Val{Num(1), Num(2), Num(3)}}, `[1,2,3,]`, `[1 2 3]`, `[1,2,3]`},
		{&List{Vals: []Val{Num(1), Num(2), Num(3)}}, `[1 2 3]`, ``, `[1,2,3]`},
		{&Dict{Keyed: []KeyVal{{"a", Num(1)}, {"b", Num(2)}, {"c", Num(3)}}},
			`{"a":1,"b":2,"c":3}`,
			`{a:1 b:2 c:3}`, ``,
		},
		{&Dict{Keyed: []KeyVal{{"a", Num(1)}, {"b", Num(2)}, {"c", Num(3)}}},
		...
		{&Dict{Keyed: []KeyVal{{"a", Num(1)}, {"b c", Num(2)}, {"+foo", Char("bar")}}},
			`{a:1, "b c":2 '+foo':'bar'}`,
			`{a:1 'b c':2 '+foo':'bar'}`,
			`{"a":1,"b c":2,"+foo":"bar"}`,
		},
